...
**Sequencing note:** Payroll's calculation engine has a hard dependency on Phase 9's Insurance/Tax calculators — implemented alongside Phase 8 (see `InsuranceCalculator` / `TaxCalculator`).

- [x] HR-089 — Create `PayrollService::openPeriod()` (P0)
...
- [x] HR-090 — Implement per-employee aggregation: Attendance → present/absent/late/early-leave (P0)
...
- [x] HR-091 — Implement per-employee aggregation: Leave → `leave_days_paid`/`leave_days_unpaid` (P0)
...
- [x] HR-092 — Implement per-employee aggregation: Overtime → minutes + earnings (P0)
...
- [x] HR-093 — Implement per-employee aggregation: Salary → `base_salary`, `earnings[]`, `deductions[]`, `gross_earnings`, `gross_salary` (P0)
...
- [x] HR-094 — Integrate Insurance calculation into `PayrollCalculator` (P0)
...
- [x] HR-095 — Integrate Tax calculation into `PayrollCalculator` (P0)
...
- [x] HR-096 — Integrate Loan deduction into `PayrollCalculator` (P0)
...
- [x] HR-097 — Implement idempotent `PayrollService::calculate(PayrollPeriod)` orchestration (P0)
  - Area: `PayrollService`
  - Acceptance:
...
- [x] HR-098 — Implement `PayrollService::approve(PayrollPeriod)` gated by `PayrollPeriodStatus::canApprove()` (P0)
  - Area: `PayrollService`
  - Acceptance:
...
- [x] HR-099 — Implement `PayrollPeriodStatus::canEdit()` enforcement across every payroll mutation path (P0)
...
- [x] HR-100 — Add a proper enum cast for `PayrollRecord.status` (P1)
...
- [x] HR-101 — Implement `PayrollService::markPaid(PayrollPeriod)` (P1)
  - Area: `PayrollService`
  - Acceptance:
...
- [x] HR-102 — Write the Payroll domain test suite, including a golden-master full-pipeline test (P0)
