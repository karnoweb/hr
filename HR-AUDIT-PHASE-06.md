...
- [ ] HR-069 — Implement "exactly one current salary per employee" as a DB-enforced invariant (P0)
...
- [ ] HR-070 — Create `SalaryService::changeSalary()` atomically closing the previous current salary and opening the new one (P0)
...
- [ ] HR-071 — Implement `CalculationType::Fixed` evaluation for `EmployeeSalaryItem`/`SalaryStructureItem` (P0)
...
- [ ] HR-072 — Implement `CalculationType::Percentage` evaluation, defining what `percentage_of` refers to (P0)
...
- [ ] HR-073 — Implement `CalculationType::Formula` evaluation with a safe, restricted expression evaluator (P1)
...
- [ ] HR-074 — Verify historical salary data remains queryable and unaffected by `changeSalary()` (P1)
...
- [ ] HR-075 — Expose `SalaryService` through the `Hr` facade (P2)
  - Area: `src/Hr.php`
  - Acceptance:
    - `Hr::salaries()` (or similar) reaches `SalaryService` consistently with the rest of the facade

- [ ] HR-076 — Propagate `SalaryItem.is_taxable`/`is_insurable` into `SalaryCalculator`'s output structure (P0)
  - Area: `SalaryCalculator`
  - Acceptance:
...
- [ ] HR-077 — Add contract-type-aware salary eligibility rules, or explicitly defer (P3)
  - Area: `SalaryService`
  - Acceptance:
...
- [ ] HR-078 — Write the Salary domain test suite (P0)
