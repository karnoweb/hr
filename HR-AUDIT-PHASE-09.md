...
**Sequencing note:** as explained in `HR-AUDIT-PHASE-08.md`, `HR-104` and `HR-106` below were implemented alongside Phase 8.

- [x] HR-103 — Design and migrate a versioned insurance-rate entity, replacing the flat config block (P0)
...
- [x] HR-104 — Create `InsuranceCalculator` consuming the versioned rate entity (P0)
...
- [x] HR-105 — Design and migrate a versioned tax-bracket entity, replacing the flat config block (P0)
...
- [x] HR-106 — Create `TaxCalculator` consuming the versioned bracket entity (P0)
  ...
  - **Policy:** monthly annualization (monthly taxable × 12 → brackets → ÷ 12).

- [x] HR-107 — Add explicit "NEEDS VERIFICATION (legal/regulatory)" flags on every hardcoded rate/bracket/exemption value (P0)
...
- [x] HR-108 — Respect `SalaryItem.is_taxable`/`is_insurable` per-item flags in both calculators (P0)
