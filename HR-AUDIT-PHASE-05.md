...
- [x] HR-061 — Create `OvertimeService` deriving `calculated_minutes` from `AttendanceRecord` (P0)
...
- [x] HR-062 — Implement night-window classification using `config('hr.overtime.night_start'/'night_end')` (P0)
...
- [x] HR-063 — Enforce `min_minutes` threshold before counting overtime (P1)
  - Area: `OvertimeService`
  - Acceptance:
    - overtime under `config('hr.overtime.min_minutes')` is not recorded/counted
    - tested at and around the exact boundary

- [x] HR-064 — Enforce `monthly_cap` on approved overtime minutes per employee (P0)
...
- [x] HR-065 — Add a proper enum cast for `OvertimeRecord.status` (P1)
...
- [x] HR-066 — Implement `requires_pre_approval` gating (P2)
  - Area: `OvertimeService`
  - Acceptance:
...
- [x] HR-067 — Expose `OvertimeService::approvedMinutesForPeriod()` for Phase 8's payroll calculator (P1)
  - Area: `OvertimeService`
  - Acceptance:
...
- [x] HR-068 — Write the Overtime domain test suite, including a unit test for the existing `OvertimeType::rate()` helper (P0)
