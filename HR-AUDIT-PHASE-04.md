...
- [ ] HR-047 — Implement leave-balance validation in `LeaveService::request()` (P0)
...
- [ ] HR-048 — Implement leave-request overlap detection (P0)
...
- [ ] HR-049 — Implement working-day/holiday-aware day counting using the shared `WorkingDayCalculator` (P0)
  - Area: `LeaveService`
  - Acceptance:
...
- [ ] HR-050 — Enforce `fixed_days` + `once_per_employment` for marriage/bereavement-style leave types (P0)
  - Area: `LeaveService`
  - Acceptance:
...
- [ ] HR-051 — Enforce `max_hours_per_month` for hourly leave (P0)
  - Area: `LeaveService`
  - Acceptance:
...
- [ ] HR-052 — Enforce `requires_document` at approval time for sick leave (P1)
...
- [ ] HR-053 — Implement `carry_over`/`carry_over_max` application at year-boundary (P1)
...
- [ ] HR-054 — Implement `LeaveService::approve()`/`reject()`/`cancel()` with atomic balance decrement on approval (P0)
  - Area: `LeaveService`
  - Acceptance:
...
- [ ] HR-055 — Implement termination-time leave-balance handling (P1)
...
- [ ] HR-056 — Implement `MissionRequest` overlap detection (with other missions and with leave) (P1)
...
- [ ] HR-057 — Wire `MissionRequest` into `DocumentType::Mission` / the workflow engine (P1)
...
- [ ] HR-058 — Auto-mark `AttendanceRecord.status` for approved leave/mission date ranges (P1)
...
- [ ] HR-059 — Expose full Leave + new Mission operations through the `Hr` facade (P2)
  - Area: `src/Hr.php`
  - Acceptance:
...
- [ ] HR-060 — Write the Leave + Mission domain test suite (P0)
