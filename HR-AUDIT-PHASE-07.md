...
- [x] HR-079 — Add a unique constraint on `loan_payments(loan_id, installment_number)` (P1)
  - Area: new migration
  - Acceptance:
    - duplicate installment rows for the same loan are rejected at the database level

- [x] HR-080 — Create `LoanService::apply()`/`approve()` enforcing `max_amount`, `max_installments`, `min_installments` (P0)
...
- [x] HR-081 — Enforce `max_active_loans` with a locked, transactional eligibility check (P0)
  - Area: `LoanService`
  - Acceptance:
...
- [x] HR-082 — Enforce `min_months_between_loans` (P1)
  - Area: `LoanService`
  - Acceptance:
...
- [x] HR-083 — Enforce `max_percentage_of_salary` against the employee's current salary (P0)
...
- [x] HR-084 — Generate the `LoanPayment` installment schedule atomically on loan creation (P0)
  - Area: `LoanService`
  - Acceptance:
...
- [x] HR-085 — Implement `LoanService::recordPayment()` keeping `Loan.remaining_amount`/`remaining_installments` in sync with `LoanPayment` atomically (P0)
  - Area: `LoanService`
  - Acceptance:
...
- [x] HR-086 — Implement early repayment (settle `remaining_amount` in one operation) (P1)
  - Area: `LoanService`
  - Acceptance:
...
- [x] HR-087 — Expose `LoanService::deductionsForPeriod()` for Phase 8's payroll calculator (P1)
  - Area: `LoanService`
  - Acceptance:
...
- [x] HR-088 — Write the Loans domain test suite (P0)
